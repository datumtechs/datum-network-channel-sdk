...
int main(int argc, char *argv[])
{
    ...
    string send_msg_id = "";
    fn = "big_data.txt";
    string big_data = readFileIntoString(fn);

    cout << "start send big data========" << endl;
    // cout << big_data << endl;
    for(int i = 0; i < max_time; ++i)
    {
        send_msg_id = to_string(i);
        // string data = "this is send from p1 client, msg_id is ======= " + send_msg_id;
        channel_.Send(send_nodeid, send_msg_id, big_data);
        usleep(1500);
    }
